...
# Per ADR 0004, Bivy strips the OTEL-based telemetry while preserving the
...
#
# Reasons:
# - Self-hosted Bivy customers should not phone home.
...
def _create_otlp_metric_exporter():
    """No-op stub. Replaced by Bivy strip 0001 (ADR 0004)."""
    return None
...
def _collect_and_push_metrics() -> None:
    ...
    Bivy does not push telemetry. If you want your own self-host telemetry,
    ...
    """
    logger.debug("Telemetry push skipped — Bivy stripped this per ADR 0004.")
    return None


@shared_task
def push_instance_metrics():
    """No-op Celery task. Replaced by Bivy strip 0001 (ADR 0004).
    ...
    """
    logger.debug("push_instance_metrics called — no-op (Bivy strip 0001).")
    return None
